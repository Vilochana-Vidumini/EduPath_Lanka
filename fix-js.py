...
# Restore file to main version to undo the syntax error
os.system("git restore student-dashboard.js")

with codecs.open('c:/dev/edupath_lanka/student-dashboard.js', 'r', encoding='utf-8', errors='ignore') as f:
    content = f.read()

# 1. Update recalculateStudentRecommendations
...
# 2. Append functions using aliases from imports
render_funcs = """
window.renderPersonalizedRecommendations = function() {
    ...
    const recommendedMentors = sharedRecommendMentors(profile, state.mentors);
...
with codecs.open('c:/dev/edupath_lanka/student-dashboard.js', 'w', encoding='utf-8') as f:
    f.write(content)
print("Updated student-dashboard.js safely using python.")
